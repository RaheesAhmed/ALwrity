...
def check_api_keys():
    ...
    missing_keys = {key: url for key, url in api_keys.items() if os.getenv(key) is None}
...
def sidebar_configuration():
    ...
    with st.sidebar.expander("**👷 Content Personalization**"):
        ...
        if blog_demographic == "Customize":
            custom_demographic = st.text_input(
        ...
        if blog_language == "Customize":
            custom_lang = st.text_input(
                "Enter the language of your choice",
                help="Specify the content language.",
            )
            if custom_lang:
    ...
    with st.sidebar.expander("**🩻 Images Personalization**"):
        ...
        number_of_blog_images = st.number_input(
            "Number of Blog Images", value=1, help="Number of blog images to include."
        )
    ...
    with st.sidebar.expander("**🤖 LLM Personalization**"):
        ...
        model = st.text_input(
            ...
            help="Mention which model of the above provider to use.",
        )
    ...
    with st.sidebar.expander("**🕵️ Search Engine Personalization**"):
        ...
        similar_url = st.text_input(
            "Similar URL",
            value="",
            help="A single URL that instructs search engines to give results similar to the given URL.",
        )
...
def main():
    ...
    os.environ["SEARCH_SAVE_FILE"] = os.path.join(
        os.getcwd(),
        "lib",
        "workspace",
        "alwrity_web_research",
        f"web_research_report_{datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}",
    )
    os.environ["IMG_SAVE_DIR"] = os.path.join(
        os.getcwd(), "lib", "workspace", "alwrity_content"
    )
    os.environ["CONTENT_SAVE_DIR"] = os.path.join(
        os.getcwd(), "lib", "workspace", "alwrity_content"
    )
    os.environ["PROMPTS_DIR"] = os.path.join(
        os.getcwd(), "lib", "workspace", "alwrity_prompts"
    )
    os.environ["ALWRITY_CONFIG"] = os.path.join(
        os.getcwd(), "lib", "workspace", "alwrity_config", "main_config.json"
    )
    ...
    if check_api_keys() and check_llm_environs():
        ...
        with tab6:
            ...
            st.markdown(
                "Create a collection by uploading files (PDF, MD, CSV, etc), or crawl a data source (Websites, more sources coming soon."
            )
            st.markdown(
...
def ai_writers():
    ...
    choice = st.selectbox(
        "**👇Select a content creation type:**",
        options,
        index=0,
...
def content_planning_tools():
    st.markdown(
        """**Alwrity content Ideation & Planning** : Provide few keywords to do comprehensive web research.
    ...
    options = ["Keywords Researcher", "Competitor Analysis", "Get Content Calender"]
    choice = st.radio(
        "Select a content planning tool:",
        options,
        index=0,
        format_func=lambda x: f"🔍 {x}",
    )
...
def alwrity_brain():
    ...
    st.write(
        "Choose a folder to write content on. Alwrity will do RAG on these documents. The documents can of any type, pdf, pptx, docs, txt, cs etc. Video files and Audio files are also permitted."
    )
